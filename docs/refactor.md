...
- [ ] **config キー命名の統一**: facet `cycle-seconds` vs wand `color-cycle-ms` 等、family 横断の
      ...
      **承認状況（2026-06-12 トミー）**: A 群（#1-6 単位/綴り統一）**全承認**。B 群は
      #8 ""=消音 / #9 色文法 / #10 anim 二層規則 **承認**、#7 off sentinel は議論中
      （トミー案=「key 未定義で off」/ off の予約語化への懸念）。C 群は #11 facet theme
      置き場（トミー対案=[tree]/[grid]/[rail] 直下に theme=""）・#13 line-pets 三点組
      （説明要求）・#15（=#11 と同義か確認）議論中、#12 wand border block・#14 exclude
      統一は**未回答**。実装は議論決着後に app 単位 PR で。
