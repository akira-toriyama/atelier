...
- [ ] **config キー命名の統一**: facet `cycle-seconds` vs wand `color-cycle-ms` 等、family 横断の
      ...
      **C 群** — #11: **第2段まで一気に実施**（トミー決定）: `[theme]` block（`.name`＋
      ...
      #12・#14 はマンデート（一貫性・破壊的変更 OK）に基づき提案どおり実施:
      #12 wand `[tome.decoration.border].effect` 昇格、#14 `[exclude].apps`（bundle-id glob
      配列）統一 — ⚠ halo はアプリ名→bundle-id の意味変更を伴う（トミーの live config は
      backup の上で移行）。実装は app 単位 PR（wand → halo → perch → facet、facet は
      Config 先 / View 配線後の 2 PR）。
