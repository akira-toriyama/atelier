...
## 🔖 次セッションへの handoff（2026-06-13 時点）

Phase 1.5 はほぼ完了。**残りは下記 4 件のみ**（依存順）。

### 1. facet release publish ＋ facet live config 移行 ← まずこれ（トミー判断要）
- facet main は PR-A（キー rename・**挙動不変**）＋ トミー進行中 tree 機能（#203/#204 等）を含む。
  rolling-draft（draft `v5.0.0`）を publish すると in-flight 機能も同時出荷。**publish 可否はトミーに確認**。
- publish 手順（wand/halo/perch で実証済）: `gh release edit <tag> --repo akira-toriyama/facet --draft=false --latest`
  → update-tap が `release: published` で自動発火（brew formula bump）。
- publish 後、**facet live config を移行**（backup-first・[[refactor-push-authorization]]）:
  `~/.config/facet/config.toml` の `theme = "..."` → `[theme]\nname = "..."`、
  `theme-cycle-seconds`(s) → `[theme].color-cycle-ms`(ms)、`[border].cycle-seconds`(s) →
  `[border].color-cycle-ms`(ms)、`[[exclude]]/[[assign]]` の `max_width/max_height` → kebab、
  `[tree].line-pets` のカンマ文字列があれば配列へ。⚠ per-view `[tree]/[grid]/[rail].theme` の
  **値付与は PR-B 後**（今足しても描画は不変）。wand/halo/perch は移行済（backup-20260612-195709）。

### 2. facet PR-B（per-view palette 配線）← 専用セッション推奨
...
### 3. jig（新 CLI app）の meta 適用確認
- [[jig-new-cli-app]]。theme/sill 対象外。commit-lint/taplo/build reusable caller・auto-merge・
  （config.toml があれば）キー命名規約と 1.6 TOML 共通化の対象か。

### 4. Track 3 残り ＋ Track 4 仕上げ
- Track 3: update-tap/build の reusable 化、CLAUDE.md の roadmap board 集約（下記 Track 3 参照）。
- Track 4: stale コメント一掃・`pal` global 撤去（=PR-B と関連）・最終 family review。
