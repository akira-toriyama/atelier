...
- [ ] **config キー命名の統一**: facet `cycle-seconds` vs wand `color-cycle-ms` 等、family 横断の
      ...
      **実装状況（2026-06-12）**: wand [#145](https://github.com/akira-toriyama/wand/pull/145) ✅ /
      halo [#13](https://github.com/akira-toriyama/halo/pull/13) ✅ /
      perch [#117](https://github.com/akira-toriyama/perch/pull/117) ✅ merged。
      facet PR-A [#199](https://github.com/akira-toriyama/facet/pull/199)（CI 待ち・auto-merge）。
      **残**: facet PR-B（per-view palette の View 配線）、トミー live config 移行
      （backup → 新キー書換。halo は exclude のアプリ名→bundle-id 変換も）。
