...
| **1.5. 仕上げ**（pin 衛生 / knob・pets dedup / meta 共通化） | 🔧 **ほぼ完了**（Track 0-4 出荷・family review PASS・**facet PR-B 出荷**／残=build reusable のみ） |
...
Phase 1.5 はほぼ完了。item 1・**item 2（PR-B 出荷）**・item 3・Track 3 の大半が出荷済。
**残りは Track 3 の build reusable のみ**（item 4 参照）→ その先は Phase 1.6（TOML 共通化）。
...
### 4. ✅ Track 4 仕上げ = 完了（family review PASS・2026-06-13）／Track 3 残り = build reusable のみ
- **Track 4 = 完了**: family drift 監査で FacetCore dedup・pal global・最終 review すべて PASS（上の Track 4
  参照）。glance 0.6.0 揃え＋genuine stale コメント（wand#148/perch#120）修正済。
