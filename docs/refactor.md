...
**Phase 1.5 = 完了**。item 1〜3・Track 3 全項目（build 共通化含む）・Track 4 すべて出荷済。
**次は Phase 1.6（TOML 共通化）**＝4本の TOML parser を sill 1モジュールに集約（golden round-trip test 駆動・
chord の 434行 parser を superset 参照）。残る meta 項目は `release` reusable のみ（最大・非一様で最後・部分的・任意）。
**Phase 1.6 は専用セッションで丸ごと実施**（トミー決定 2026-06-13）。

### Phase 1.6 着手ブリーフ（次セッター向け）
- **起点 = sill 側から**: chord の 434行 parser を superset 参照に、共有 TOML parser を sill の**新モジュール**として切り出し
  ＋ golden round-trip test 駆動。ここは consumer に触れず**衝突ゼロ**で進む。
- **consumer = facet / perch / wand / chord の 4 パーサ**を順に sill モジュールへ swap（各 app は §運用 ritual の push ritual
  ＝path-dep で atomic 編集 → tag/release → url+SemVer swap → 単一 app PR → auto-merge）。
- ⚠ **facet の懸念**: TOML パーサ＋config.toml を持つ対象だが **活発開発中**（[[facet-active-dev]]）。PR-B が #210/#216 で
  **2 度 rebase** した churn を踏まえ、**facet の consumer swap は最後に回す**（or 活発でない時に）。sill 土台→perch/wand/chord
  →facet の順が安全。
- ⚠ **jig の懸念**: jig は **CLI で TOML 少なめ** — `cliff.toml`（git-cliff 用）のみ・**config.toml 無し**・パーサ本体は
  **JSON**（TOML ではない）＝ **Phase 1.6 対象外**（[[jig-new-cli-app]]）。ただし別セッションで **string-interpolation の WIP が
  進行中**（本セッション時点で working tree に未コミット 11 ファイル）→ jig に触る用があれば**その WIP を壊さない**こと。
