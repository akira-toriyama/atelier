...
| **2. border 共通化**（halo/facet の BorderFX animator 統一・perch は要判断） | 📋 **着手ブリーフ済**（下記「Phase 2 計画」・grill 待ち→新セッション） |

sill 現況: **タグ 0.7.1**（2026-06-13 出荷）。Palette（pure）/ PaletteKit（AppKit）/
...
各 app の sill 採用（origin/main・Phase 1.6 後）: perch `0.7.1`（#122）/ wand `0.7.1`（#150）/
chord `0.7.1`（#75・Toml のみ・初の sill 依存）/ facet `0.7.1`（#221）/ halo `0.6.0`（#12・TOML 非消費ゆえ据置）/
...
### 想定 swap 順 / 検証
**facet → halo** の順（facet の BorderFX は既に generic class ＝抽出しやすい / halo は border が本体ゆえ慎重に最後）。
perch は論点①次第。各 swap は sill に共有 BorderFX を足す → path-dep で atomic 編集 → build → **スクショ self-verify** →
url+SemVer swap → app PR → auto-merge（§push ritual）。sill は CI 無しゆえ `swift run` ハーネス＋ホスト目視。
...
横断リファクタ本体は全て完了。残るは **Phase 2（border 共通化）のみ**＝parking 中（将来）。
...
**現在地（2026-06-13）**: **Phase 1/1.5/1.6 = 完了**（sill `Toml` 0.7.0→0.7.1・consumer swap 4/4 出荷:
perch#122 / wand#150 / chord#75 / facet#221）＋ **Track 3 完全完了**（`release` reusable 出荷・.github#4 ＋
caller 4本: halo#17 / facet#222 / perch#123 / wand#151）。**横断リファクタ本体は全完了** — theme / CI(commit-lint・
update-tap・build・release) / CLAUDE.md / TOML が全て single-sourced、drift 面消滅。**残るは Phase 2（border 共通化）のみ**＝parking。

1. ダッシュボードで残 Track を確認（Phase 1.5/1.6・Track 3/4 全て ✅）。**残るは Phase 2 のみ**。
2. **Phase 2: border 共通化** ← 次の作業（トミー GO 済・新セッション推奨）。**着手ブリーフは上記「Phase 2 計画」節**
   （調査済: 重複の正体＝halo↔facet の BorderFX アニメータ／死守＝render path／perch は要判断／sill 置き場＝Effects 拡張／
   grill 論点5つ）。着手は **grill 論点①②をトミーと詰める**ところから。
3. **sill 検証**: sill に CI が無い。テストは `swift run` の一時 executable（XCTest 相当）でホスト実行
   ＝ build green ＋ そのハーネスで確認（[[tart-vm-available]] は private GHCR 認証時のみ）。
4. **config 系 swap の検証作法**（1.6 で確立）: CLT に XCTest 無し＝`swift test` ローカル不可。各 app の
   `--validate`（無ければ一時ハーネス）で **旧 HEAD ビルドと resolved config を before/after diff** する＝
   実機バイト一致で挙動同値を証明。テスト target は CI でのみコンパイル＝削除/改名は CI 任せにせず grep で先に潰す。
5. facet は活発開発中 — 小さめマージ・最新化を意識、refactor 系 facet タスクは少しなら後回し可。
6. Track 4（最終 family review）は完了済。
