...
| **3. CLI 文法統一**（yabai 式 domain-verb・daemon-control の facet/chord/wand/perch が IN／glance・jig・atelier・halo は OUT） | 🚧 **着手ブリーフ確定**（2026-06-14・8 app 実コード調査＋敵対批評・ultracode）→ 正典 [`cli-grammar.md`](cli-grammar.md)。**横断リファクタ第4の共通化**。設計 D0-D8 はトミー委任で全確定（0ベース/破壊OK）。cornerstone=**sill `CLIKit`/D0 tokenizer** から着手 |
...
- [x] **halo 色変換 dedup** ✅（halo#12）: ローカル `NSColor(hex: String)`（6桁限定）と
      ...
      #rrggbbaa）。
- [x] **config キー命名の統一**: facet `cycle-seconds` vs wand `color-cycle-ms` 等、family 横断の
...
- **CLI 文法統一 = Phase 3（2026-06-14・トミー委任で D0-D8 確定）**: family の CLI を yabai 式
  `<tool> <domain> --<verb>` に統一する横断リファクタ第4の共通化。正典は [`cli-grammar.md`](cli-grammar.md)。
  daemon-control（facet/chord/wand/perch）が IN、data-processing（glance/jig）・orchestrator（atelier）・
  config-only-daemon（halo）は OUT で横断 sub-規約のみ整合。**確定の核**: ① **D0 tokenizer**＝`--verb=value`→空白区切り化で
  値が `-`始まり/負数だと flag と判別不能になる（jig parser が `hasPrefix("-")` で全 `-`始まりを flag 扱い＝この罠を他 app に
  展開すると現存呼出が無言 exit 2）→ 値要求 verb の直後トークンは符号付きでも値消費＋`--` 終端を **sill `CLIKit` の第一仕様**に。
  ② **D4** sill `CLIKit` は pure mechanism のみ（tokenizer/exit helper/`--show` emitter）、**verb-table validator と
  domain/verb 語彙は app local 死守**（border の perch OUT と同じ「zero-debt≠全部共有」）。③ **D2** 完全 full-replace・シム無し
  （inter-app 呼出は alias でなく呼出元を grep して同 sweep 更新）。④ **D3** 補完は別 PR（list 口＝新規 read API で文法移行を人質に
  しない）。⑤ daemon/config domain は**配置のみ**統一（`wand --resign`≠`facet --resign` 等の同名異義ゆえ verb セットは揃えない）。
  実装順＝sill CLIKit → facet（PR1 `=`→空白 / PR2 domain 再編 / PR3 `--show`+list 口）→ wand → perch → chord → OUT 各1 PR。
...
すべて single-sourced、drift 面は構造的に消滅。sill は **0.8.0**。**次フロンティア = Phase 3（CLI 文法統一）に着手**
（正典 [`cli-grammar.md`](cli-grammar.md)・設計 D0-D8 確定済・cornerstone=sill `CLIKit`/D0 tokenizer）。
...
- **次にやる = Phase 3（CLI 文法統一）**: 正典 [`cli-grammar.md`](cli-grammar.md)。cornerstone の sill `CLIKit`
  （pure tokenizer / D0 仕様）から着手 → facet → wand → perch → chord → OUT 各1 PR。設計 D0-D8 はトミー委任で確定済。
  （さらに先の新プロダクト = rofi 概念の picker [[atelier-picker-rofi-direction]] は当面先送り）。
