...
残るは **任意の meta `release` reusable のみ**（Track 3・最後・部分的・未着手）。Phase 2（border 共通化）は parking。
...
**現在地（2026-06-13）**: **Phase 1.5 = 完了** / **Phase 1.6 = 完了**（sill `Toml` 0.7.0→0.7.1・
consumer swap 4/4 出荷: perch#122 / wand#150 / chord#75 / facet#221）。4本の手書き TOML パーサは
消滅し全 app が sill の `Toml` を参照。**残るリファクタ作業は 任意の meta `release` reusable のみ**
（Track 3・最後・部分的・非一様で最大ゆえ慎重に・未着手）。Phase 2（border 共通化）は parking。

1. ダッシュボードで残 Track を確認（Phase 1.5/1.6 は全て ✅）。
2. **任意: `release` reusable**（Track 3 の最後）— glance/chord/jig は意図的に divergent（[released]・
   latest-fetch・厳格 validation・専用 bot・emoji commit）ゆえ非対象。facet/halo/perch/wand の release.yml が
   どこまで一様か精査 → 共通部分だけ composite/reusable 化（決定ログ「CI 共通化の手段選択」と同じ判定軸:
   PR ゲートか＆caller が前後ステップを足すか）。ROI 低めゆえ後回し可。
